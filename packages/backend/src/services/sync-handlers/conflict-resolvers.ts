...
import type { IConflictResolver } from "@avileo/drizzle-sync/server";
...
abstract class BaseVersionConflictResolver implements IConflictResolver<RequestContext, DbTransaction> {
...
class VersionConflictResolver implements IConflictResolver<RequestContext, DbTransaction> {
  async checkConflict(
    ctx: RequestContext,
    operation: SyncOperationInput,
    tx: DbTransaction
...
class SaleItemConflictResolver implements IConflictResolver<RequestContext, DbTransaction> {
  async checkConflict(
    ctx: RequestContext,
    operation: SyncOperationInput,
    tx: DbTransaction
...
export function createConflictResolvers(): Record<string, IConflictResolver<RequestContext, DbTransaction>> {
  return {
...
class ConflictResolverAdapter implements IGenericConflictResolver<RequestContext, DbTransaction, string> {
  constructor(
    private resolver: IConflictResolver<RequestContext, DbTransaction>,
