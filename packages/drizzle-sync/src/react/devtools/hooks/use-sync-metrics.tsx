...
import { getEventBuffer, type TimelineEvent } from "../../../core/event-buffer";
...
export function useSyncMetrics(): UseSyncMetricsReturn {
  ...
  const calculateMetrics = useCallback(() => {
    ...
    const totalOps = syncState.pendingCount + syncState.failedCount + syncState.conflictCount + syncState.deadLetterCount;
    const dlqRate = totalOps > 0 ? (syncState.deadLetterCount / totalOps) * 100 : 0;
    ...
    const queueAge: number | null = syncState.pendingCount > 0 ? Math.floor(Math.random() * 5) + 1 : null;
  ...
  return { metrics, timeWindow, setTimeWindow, isLoading };
}
