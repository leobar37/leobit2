...
export function generateHooksFile(
  ...
  b.line("// Uses CUID2 for frontend ID generation");
  b.blank();
  b.line('import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";');
  b.line('import { api } from "~/lib/api-client";');
  b.line('import { createId } from "@paralleldrive/cuid2";');

  if (entityNames.length > 0) {
    b.line("import {");
    b.indent((ib) => {
      entityNames.forEach((entityName) => ib.line(`${entityName}Schema,`));
    });
    b.line('} from "./schemas";');
  } else {
    b.line('import {} from "./schemas";');
  }

  if (allInputTypes.length > 0) {
    b.line("import type {");
    b.indent((ib) => {
      allInputTypes.forEach((inputType) => ib.line(`${inputType},`));
    });
    b.line('} from "./types";');
  } else {
    b.line('import type {} from "./types";');
  }

  b.line('import { generateIdempotencyKey } from "@avileo/shared";');
  b.blank();
