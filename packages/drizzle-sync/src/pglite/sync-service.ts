 ...
 * Manages the sync queue, HTTP client, lifecycle, and auto-sync.
 *
 * This is a generic implementation that can be used in any application
 * with any HTTP client implementation.
 */
...
import type {
  ...
  EnqueueParams,
  ISyncHttpClient as CoreISyncHttpClient,
  HandlerResult,
} from "../core";
import { CONFLICT_STRATEGY, SYNC_INTERVAL_MS } from "../shared/constants";
import type { ConflictStrategy } from "../shared/constants";
...
import { SyncMutex, type ISyncMutex } from "./sync-mutex";
import type { ISyncLogger } from "../core";
import { parsePayload } from "../core";

// Re-export batch processor types
export type {
...
/**
 * Extended HTTP client interface for sync operations.
 *
 * Extends the core ISyncHttpClient with additional methods for conflict management.
 * This is used by SyncService for its specific needs.
 */
export interface ISyncHttpClient extends CoreISyncHttpClient {
  /**
...
/**
 * Options for SyncService construction
 */
export interface SyncServiceOptions {
  /**
   * Custom sync queue implementation.
   * If not provided, a PgSyncQueue will be created.
   */
  queue?: ISyncQueue;

  /**
   * Custom HTTP client implementation.
   * REQUIRED - must be provided by the application.
   */
  httpClient?: ISyncHttpClient;

  /**
   * Custom sync mutex implementation.
   * If not provided, a new SyncMutex will be created.
   */
  mutex?: ISyncMutex;
  ...
  /**
   * Custom initialization service options.
   */
...
/**
 * Backend conflict types for API responses
 */
 ...
 * Sync Service
 *
 * Main entry point for push sync operations.
 * Coordinates queue management, HTTP communication, lifecycle tracking,
...
export class SyncService {
  ...
  constructor(
    ...
    this.initializationService = new SyncInitializationService(
      pg,
      businessId,
  ...
  async retryAllDeadLetterOperations(): Promise<number> {
    ...
    for (const operation of deadLetterOperations) {
      const success = await this.lifecycle.retryDeadLetterOperation(operation.id);
      if (success) {
        retried += 1;
      }
    }
  ...
  async cleanup(): Promise<void> {
    this.stopAutoSync();
    // Note: Cleanup service is app-specific (uses localStorage)
    // Applications should implement their own cleanup logic
  }
...
/**
 * Factory function to create a new SyncService instance.
 */
export function createSyncService(
  ...
  return new SyncService(pg, businessId, options);
}
