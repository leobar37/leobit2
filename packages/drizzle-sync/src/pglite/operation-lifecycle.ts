...
import { syncLogger } from "./compat";
import type { SyncEntityStatusUpdater } from "./entity-status-updater";

export interface OperationLifecycleOptions {
  /** Column name for tenant filtering (default: "tenant_id") */
  tenantColumn?: string;
  /** Set of entity types that support self-healing (update → create conversion) */
  selfHealRules?: Set<string>;
  /** Custom error classifier (default: built-in classifyError) */
  errorClassifier?: (error: string) => ClassifiedError;
}

/**
 * Manages operation state transitions: pending → processing → completed/failed/conflict/deadLetter.
 * Supports configurable self-heal rules for update→create conversion.
 */
export class SyncOperationLifecycleService {
  ...
  async retryDeadLetterOperation(deadLetterId: string): Promise<boolean> {
    ...
    const updated = await this.adapter.query<{ id: string }>(
      `UPDATE sync_operations
       ...
       RETURNING id`,
      [OPERATION_STATUS.PENDING, record.operation_id, this.tenantId]
    );
  ...
  async logDetailedStatus(): Promise<void> {
    console.log(`[SYNC] Detailed Queue Status for tenant: ${this.tenantId}`);
  ...
  private async trySelfHealOperation(
    ...
    if (
      ...
      !this.selfHealRules.has(operation.entity_type) ||
    ...
    await this.adapter.exec(
      `UPDATE sync_operations
       SET operation = $1,
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $3
         AND "${this.tenantColumn}" = $4`,
      ["create", OPERATION_STATUS.PENDING, operation.id, this.tenantId]
    );
