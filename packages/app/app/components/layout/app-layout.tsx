...
/**
 * @deprecated Route-level shell composition should be declared directly with
 * `MobileShell` + `MobileSlot` (for example: `header:left`, `header:center`,
 * `header:right`) inside route JSX.
 *
 * This hook remains as a compatibility bridge while protected routes finish
 * migration to the route-level mobile shell composition API.
 */
...
export function AppLayout({ children, headerAccessory }: AppLayoutProps) {
  ...
  const headerControls = (
    <AppLayoutHeaderControls>
      {actions ? <AppLayoutHeaderControl>{actions}</AppLayoutHeaderControl> : null}
      {headerAccessory ? (
        <AppLayoutHeaderControl>{headerAccessory}</AppLayoutHeaderControl>
      ) : null}
